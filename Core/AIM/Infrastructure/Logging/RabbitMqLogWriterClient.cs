namespace UBIS.Services.Aim.Infrastructure.Logging;
...
public class RabbitMqLogWriterClient : ILogWriterClient
{
    ...
    private async Task PublishAsync(string level, string message, Exception? exception, object? properties, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // e.g. "log.information" / "log.warning" / "log.error" — LogWriter binds with pattern "log.*".
        ...
        {
            // Never let a logging-pipe outage break authentication/OTP flows; fall back to the local sink.
