namespace UBIS.Services.Sbe.WebApi.Security;
...
/// <summary>
/// FR-006 (Token Refresh &amp; Revocation): a cryptographically valid, unexpired JWT is still
/// rejected once its session's <c>ubis:session:{sid}</c> Redis key is gone — deleted by AIM's
/// force-logout, by a password change revoking every outstanding session, or simply expired via
/// its own sliding TTL. Every JWT-validating microservice registers this same events object
/// (duplicated per service, matching how JwtOptions/JwtValidationFactory are already duplicated
/// rather than shared, copied from ECL's own JwtSessionRevocationEvents) so a revoked token is
/// rejected everywhere, not just by AIM itself. Lives in the WebApi project, not Infrastructure —
/// JwtBearerEvents requires the Microsoft.AspNetCore.Authentication.JwtBearer package, only
/// referenced here.
/// </summary>
public static class JwtSessionRevocationEvents
{
    public static JwtBearerEvents Create() => new()
    {
        OnTokenValidated = async context =>
        {
            ...
            // Fails OPEN (allows the request through) rather than closed if Redis itself is
            // unreachable - see ECL's JwtSessionRevocationEvents for the confirmed real-world
            // failure mode this avoids (an unhandled RedisConnectionException here previously
            // broke every authenticated request to every service, the moment Redis was
